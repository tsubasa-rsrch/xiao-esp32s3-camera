...
// 静的IPアドレス設定（リビング用）
IPAddress staticIP(10, 0, 72, 204);  // リビング
IPAddress gateway(10, 0, 0, 1);      // キッチンと同じゲートウェイ
IPAddress subnet(255, 255, 0, 0);    // /16 サブネット
...
bool initCamera() {
  ...
  if (err == ESP_OK) {
    // カメラの向き設定（設置が上下逆の場合の補正）
    sensor_t * s = esp_camera_sensor_get();
    s->set_vflip(s, 1);  // 垂直反転ON
  }
  return (err == ESP_OK);
}
